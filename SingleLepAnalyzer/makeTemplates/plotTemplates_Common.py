...
iPlot='lepPt'
if whichSig == 'X53H' and region=='SR' and 'XGB' in iPlot: doSig =False
...
if len(sys.argv)>1: 
	...
	if whichSig == 'X53H': templateDir2=os.getcwd()+'/'+str(sys.argv[1])+'_X53H/'
	if whichSig == 'X53H' and 'XGB' in iPlot: templateDir2=os.getcwd()+'/'+str(sys.argv[1])+'/'


else:
    templateDir=os.getcwd()+'/kinematics_R17_SR_2024_3_24/'
    if whichSig == 'X53H': templateDir2 = os.getcwd()+'/kinematics_R17_SR_2024_3_24/'#'/kinematics_R17_sinancuts_PS_2024_3_10/'#'/kinematics_CR_2024_2_13/'#'/templates_R18_SR_2024_2_14/'#'/kinematics_CR_2024_2_13/'#'/templates_SR_2024_2_13/'#'/templates_2023_11_9/'
...
if whichSig == 'X53H':
    sig1='X53M'+massPt+'MH'+massPtH2 # choose the 1st signal to plot
    #sig1='X53RHM'+massPt # choose the 1st signal to plot

    #sig1leg='X_{5/3}#bar{X}_{5/3} (tW) ('+massPt+' GeV)'
    M =  massPt
    sig1leg='X_{5/3}#bar{X}_{5/3} ('+massPt+' GeV) H^{+} ('+massPtH2+' GeV)'
...
if region=='SR' or region=='CR' or region=='':scaleSignals = False#True ##check
scaleFact1 = 100
scaleFact2 = 100
scaleFact3 = 100
scaleFact4 = 100
...
def formatUpperHist(histogram):
	...
	if year == 'R16' and 'XGB200_SR1' in histogram.GetName(): histogram.GetXaxis().SetTitle("XGB SR1 (MX 1200 GeV MH+ 200 GeV)")
	if year == 'R16' and 'XGB400_SR1' in histogram.GetName(): histogram.GetXaxis().SetTitle("XGB SR1 (MX 1200 GeV MH+ 400 GeV)")
	if year == 'R16' and 'XGB600_SR1' in histogram.GetName(): histogram.GetXaxis().SetTitle("XGB SR1 (MX 1200 GeV MH+ 600 GeV)")
	if year == 'R16' and 'XGB800_SR1' in histogram.GetName(): histogram.GetXaxis().SetTitle("XGB SR1 (MX 1200 GeV MH+ 800 GeV)")
...
if whichSig == 'X53H':
    ...
    print RFile2
    RFile3 = rt.TFile(templateDir2+tempsig)#.replace('_wNegBinsCorrec_',''))
    RFile4 = rt.TFile(templateDir2+tempsig)
    RFile5 = rt.TFile(templateDir2+tempsig)
    RFile6 = rt.TFile(templateDir2+tempsig)

if plotttnobb: 
	RFile2 = rt.TFile(templateDir2+tempsig) 
	print RFile2
#set the tdr style
...
for tag in tagList:
	...
	postTag = '' 
	if 'CR' in templateDir: postTag = 'isCR_'
	if 'SR' in templateDir: postTag = 'isSR_'
#                blind = False
#	else: 
#		postTag = 'isSR_'
#		blind = blindGlob
...
#	if not isCategorized: blind = blindGlob
	...
	for isEM in isEMlist:
		...
		if plotCombine:
			...
			print "sig1", sig1
			hsig1 = RFile1.Get(histPrefix+'__'+sig1).Clone(histPrefix+'__sig1')#HERE
			#hsig1 = RFile1.Get(histPrefix+'__'+sig1).Clone(histPrefix+'__ttnobb')

			if whichSig == 'X53H':
				#hsig2 = RFile2.Get(histPrefix+'__'+sig2).Clone(histPrefix+'__ttnobb')
			...
			#hsig1 = RFile1.Get(histPrefix+'__sig1').Clone(histPrefix+'__ttnobb')

			if whichSig == 'X53H':
				hsig2 = RFile2.Get(histPrefix+'__sig2').Clone(histPrefix+'__sig2')
				#hsig2 = RFile2.Get(histPrefix+'__sig2').Clone(histPrefix+'__ttnobb')
				...
				hsig6 = RFile6.Get(histPrefix+'__sig6').Clone(histPrefix+'__sig6')
...
#reapplies the xsec just for plotting--I turned of scale to 1pb in doTemplates
		...
		if doNormByBinWidth:
			for proc in bkgProcList:
				...
				if plotttnobb: 
					try: normByBinWidth(bkghists2['ttnobb2'+catStr])
					except: pass
...
#Turned off scale to 1pb in doTemplates
	...
	if doNormByBinWidth:
		for proc in bkgProcList:
			...
			if plotttnobb:
				try: normByBinWidth(bkghistsmerged2['ttnobb2'+'isL'+tagStr])
				except: pass
...
# 		normByBinWidth(hsig2merged)
		...
		if plotttnobb: normByBinWidth(hDatamerged2)
		if whichSig == 'X53H':
 			normByBinWidth(hsig2merged)
...
#   	
	...
	if not blind: 
		#if 'XGB' in iPlot and region == 'CR':hDatamerged.GetXaxis().SetRangeUser(0,0.95)
...
# 	hsig2merged.Draw("SAME HIST")
 ...
 #	legmerged.SetTextFont(62)
	if whichSig == 'X53H' and region=='CR': legmerged.SetTextSize(.03)
	if whichSig == 'X53H' and region=='SR': legmerged.SetTextSize(.019)
