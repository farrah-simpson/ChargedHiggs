...
def add_processes_and_observations(cb, prefix='X53M'):
...
def add_shapes(cb):
	...
	for chn in chns:
		...
		sig_pattern = iPlot+'_'+lumiStr+'_%s$BIN__$PROCESS$MASSMH400' % chn
		if 'isCR' not in chn:
...
def rename_and_write(cb):
	...
	writer = ch.CardWriter('limits_'+signal+'_'+massH+'_'+template+saveKey+'/$TAG/$MASS/$ANALYSIS_$CHANNEL_$BINID_$ERA.txt',
...
def add_standard_systematics(cb):
	...
	#cb.cp().process(signal + allbkgs).channel(chns).AddSyst(cb, 'CFerr1_$ERA', 'shape', ch.SystMap('era')(['R16'], 1.0)(['R17'], 1.0)(['R18'], 1.0))
       	#cb.cp().process(signal + allbkgs).channel(chns).AddSyst(cb, 'tau32', 'shape', ch.SystMap('era')(['R16'], 1.0)(['R17'], 1.0)(['R18'], 1.0))
	...
	#cb.cp().process(signal + allbkgs).channel(chns).AddSyst(cb, 'btag', 'shape', ch.SystMap('era')(['R16'], 1.0)(['R17'], 1.0)(['R18'], 1.0)) # Uncorrelated; Ex: B2G-19-001/AN2018_322_v7
	#cb.cp().process(signal + allbkgs).channel(chns).AddSyst(cb, 'mistag', 'shape', ch.SystMap('era')(['R16'], 1.0)(['R17'], 1.0)(['R18'], 1.0)) # Uncorrelated; Ex: B2G-19-001/AN2018_322_v7
	...
	#		cb.cp().process([proc]).channel(chns).AddSyst(cb, 'isr_tt', 'shape', ch.SystMap()(1.0))
	#		cb.cp().process([proc]).channel(chns).AddSyst(cb, smoothAlgo+'fsr_tt', 'shape', ch.SystMap()(1.0))
	#	else: 
	...
	#		cb.cp().process([proc]).channel(chns).AddSyst(cb, 'isr', 'shape', ch.SystMap()(1.0))
	#		cb.cp().process([proc]).channel(chns).AddSyst(cb, 'fsr', 'shape', ch.SystMap()(1.0))
...
def create_workspace(cb):
	...
	for chn in ['cmb']:#+chns:
		for mass in masses:
			chnDir = os.getcwd()+'/limits_'+signal+'_'+massH+'_'+template+saveKey+'/'+chn+'/'+mass 
...
if __name__ == '__main__':
	...
	bkg_procs = {chn:[hist.split('__')[-1] for hist in allHistNames if '_'+chn+'_' in hist and not (hist.endswith('Up') or hist.endswith('Down') or hist.endswith(dataName) or hist.find('X53')!=-1)] for chn in chns}
