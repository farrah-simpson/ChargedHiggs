...
def analyze(tTree,process,flv,cutList,doAllSys,doPDF,iPlot,plotDetails,catStr,region,year):
	...
	isEM  = catStr.split('_')[0][2:]
	#nhott = catStr.split('_')[1][4:]
	nttag = catStr.split('_')[1][2:]
	nWtag = catStr.split('_')[2][2:]
	nbtag = catStr.split('_')[3][2:]
	njets = catStr.split('_')[4][2:]
...
#	print 'Flavour: '+isEM+' #hott: '+nhott+' #ttags: '+nttag+' #Wtags: '+nWtag+' #btags: '+nbtag+' #jets: '+njets
	...
	if doAllSys:
		systList = ['pileup','muRFcorrd','muR','muF','isr','fsr','tau32','jmst','jmrt','tau21','jmsW','jmrW','tau21pt','btag','mistag','njet','njetsf', 'CSVshapelf', 'CSVshapehf']#,'toppt'
